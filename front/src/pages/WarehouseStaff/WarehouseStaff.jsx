...
const SHIPPING_STATUSES = ['pending', 'completed', 'cancelled']
...
export default function WarehouseStaff() {
  ...
  useEffect(() => {
    if (isLoading) return
    if (!user || user.role !== 'warehouse_staff') { navigate('/'); return }
    loadOrders()
  }, [user, isLoading])
  ...
  async function loadOrders() {
    ...
    setOrders(list.filter(o => o.deliveryType === 'shipping'))
    setLoading(false)
  }
    ...
    <article className="staff">
      <header className="staff__header">
        <div className="staff__header-content">
          <h1 className="staff__title">WAREHOUSE STAFF</h1>
          <p className="staff__subtitle">Shipping Order Processing</p>
        </div>
            ...
            <div className="staff__actions">
              {selectedOrder.status === 'pending' && (
                <button
                  ...
                  onClick={() => changeStatus(selectedOrder._id, 'ready_to_ship')}
                >
                  SEND TO COURIER
                </button>
              )}
